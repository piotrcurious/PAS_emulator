/*
  ...
  - Power-saving sleep mode with wake-on-throttle functionality.
...
const int WAKEUP_PIN = 14 ;     // you can now use any pin thanks to PCINT , also same pin as the throttle
...
const int AUDIO_MULTIPLIER = 5; // Factor to convert PAS frequency to audible range
...
struct SleepState {
  bool enabled;
  unsigned long lastActivityMicros;
...
/**
 * @brief Interrupt Service Routine for wake-up on throttle pin change.
 */
void wakeUpISR() {
  wakeUpFlag = true;
}
...
void updateSerialOutput(unsigned long currentMicros) {
  if (currentMicros - lastSerialOutput >= SERIAL_INTERVAL) {
    ...
    Serial.print(" Hz | Sleep in: ");
    Serial.print((SLEEP_TIMEOUT - (currentMicros - sleepManager.lastActivityMicros)) / 1000000);
    Serial.println(" sec");
 ...
 * @param currentMicros Current microsecond timestamp.
 */
void processThrottleInput(unsigned long currentMicros) {
  throttleValue = analogRead(THROTTLE_PIN);
  bool throttleActive = (throttleValue > throttlePoints[0]);
  
  // Update activity timer if throttle state changes or is active
  if (throttleActive != sleepManager.throttleWasActive || throttleActive) {
    sleepManager.lastActivityMicros = currentMicros;
    sleepManager.throttleWasActive = throttleActive;
  }
...
void setupThrottleForWakeup() {
  ...
  PcInt::attachInterrupt(WAKEUP_PIN, wakeUpISR, RISING);
}

/**
 * @brief Restores the throttle pin to analog input mode.
 */
void restoreThrottleAnalog() {
  // Detach interrupt
  PcInt::detachInterrupt(WAKEUP_PIN);
...
/**
 * @brief Enters power-saving sleep mode.
 */
void enterSleepMode() {
  if (DEBUG_MODE) {
  ...
  // Configure throttle pin for wake-up
  setupThrottleForWakeup();
  ...
  // Disable brown-out detector during sleep (if supported)
  #ifdef BODS
  sleep_bod_disable();
  #endif
  ...
  // Restore throttle pin to analog input
  restoreThrottleAnalog();
  
  // Reset activity timer
  sleepManager.lastActivityMicros = micros();
...
/**
 * @brief Checks if sleep mode should be activated and handles the transition.
 * @param currentMicros Current microsecond timestamp.
 */
void manageSleepMode(unsigned long currentMicros) {
  // Check if we should enter sleep mode
  if (currentMicros - sleepManager.lastActivityMicros >= SLEEP_TIMEOUT) {
    // Only enter sleep if throttle is currently inactive
    if (throttleValue <= throttlePoints[0]) {
      enterSleepMode();
    } else {
      // Reset timer if throttle became active again
      sleepManager.lastActivityMicros = currentMicros;
    }
  }
}
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("Advanced PAS Emulator with Sleep Mode Initialized.");
  Serial.println("=================================================");
  ...
  Serial.print(SLEEP_TIMEOUT / 1000000);
  ...
  pas = {false, 0, 0, 0, 0};
  buzz = {false, 0, 0};
  sleepManager = {true, micros(), false};
  lastSerialOutput = 0;
  wakeUpFlag = false;
}
...
void loop() {
  ...
  processThrottleInput(currentMicros);
